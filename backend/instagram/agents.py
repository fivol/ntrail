...
class WebAgentAccount(Account, WebAgent):
    ...
    def auth(self, password, settings=None):
        ...
        if self.logger:
            self.logger.debug('AUTH response is %s', response)
    ...
    @exception_manager.decorator
    def get_followers(self, account=None, pointer=None, count=20, limit=50, delay=0, query_hash=None, settings=None):
        if account is None:
            account = self
        if not self.logger is None:
        ...
        while True:
            ...
            response = self.graphql_request(
                query_hash="37479f2b8209594dde7facb0d904896a" if not query_hash else query_hash,
                variables=variables_string.format(**data),
                referer="https://instagram.com/%s%s" % (
                    account.base_url,
                    getattr(account, account.primary_key),
